...
Log in to SDE virtual desktop using your CNetID credential as described in [connecting chapter](connection.md). Please note that only SDE2 users have access to the SDE2 HPC system. Having an RCC account on Midway ecosystem, such as Midway2 or Midway3, does not automatically grant you an account on SDE2.
...
## Step 2. From SDE Virtual Desktop to SDE2
After the regulated data has been transferred from the Data Provider to the SDE virtual desktop, open the WinSCP application. While WinSCP can be used to connect to SDE2, connecting to any non-UChicago hosts is currently prohibited. Therefore, it cannot be used for the data transfer described in Step 1. <br><br>
